...
| `owb skill install [--to <agents>] [--project] [--dir <path>]` | Install the skill for **every detected agent** — known ones (`~/.claude`, `~/.codex`, `~/.kimi`, `~/.qwen`, `~/.iflow`, `~/.gemini`, `~/.config/opencode`, `~/.cursor`, `~/.agents`) plus any `~/.<tool>/skills/` or `~/.config/<tool>/skills/` that already exists. `--to kimi,codex` picks agents (any name works; an unknown one goes to `~/.<name>/skills/`), `--project` installs into the current project instead of the home directory, `--dir <path>` installs to `<path>/owb`, `--link` symlinks the package's skill source instead of copying it, so `npm i -g open-web-bridge@latest` updates it with no second step (falls back to a copy where symlinks are unavailable) |
| `owb skill print` | Print `SKILL.md` to stdout — for agents that have no skills directory, paste it into whatever rules file they do read (`owb skill path` locates the `references/`) |
