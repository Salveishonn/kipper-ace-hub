...
const ProductorEmisiones = () => {
  ...
  // Get emissions for selected month
  const [year, month] = selectedMonth.split('-').map(Number);
  const monthStart = startOfMonth(new Date(year, month - 1));
  const monthEnd = endOfMonth(new Date(year, month - 1));

  const emissions = myPolicies.filter(p => 
    isWithinInterval(new Date(p.created_at), { start: monthStart, end: monthEnd })
  );

  const filteredEmissions = emissions.filter(p =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Emisiones</h1>
          <p className="text-muted-foreground">Pólizas emitidas por mes</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-green-100 text-green-700 px-4 py-2 rounded-full">
            <FileCheck size={18} />
            <span className="font-bold">{filteredEmissions.length}</span>
      ...
      {filteredEmissions.length === 0 ? (
        <div className="bg-card rounded-2xl shadow-soft p-12 text-center">
          <FileCheck size={48} className="mx-auto text-muted-foreground mb-4" />
          <p className="text-muted-foreground">No hay emisiones en este período</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="text-left p-4 text-sm font-medium text-muted-foreground">Fecha Emisión</th>
                <th className="text-left p-4 text-sm font-medium text-muted-foreground">Prima</th>
              </tr>
            </thead>
            <tbody>
              {filteredEmissions.map((policy) => (
...
export default ProductorEmisiones;
